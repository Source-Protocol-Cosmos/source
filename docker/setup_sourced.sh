...
GENESIS_FILE="$HOME"/.source/config/genesis.json
if [ -f "$GENESIS_FILE" ]; then
  ...
  sed -i 's/keyring-backend = "os"/keyring-backend = "test"/' "$HOME"/.source/config/client.toml
fi

APP_TOML_CONFIG="$HOME"/.source/config/app.toml
APP_TOML_CONFIG_NEW="$HOME"/.source/config/app_new.toml
CONFIG_TOML_CONFIG="$HOME"/.source/config/config.toml
...
if ! sourced keys show validator $KEYRING; then
  (echo "$PASSWORD"; echo "$PASSWORD") | sourced keys add validator $KEYRING

  # hardcode the validator account for this instance
  echo "$PASSWORD" | sourced add-genesis-account validator "1000000000$STAKE,1000000000$FEE" $KEYRING

  # (optionally) add a few more genesis accounts
  for addr in "$@"; do
    echo $addr
    sourced add-genesis-account "$addr" "1000000000$STAKE,1000000000$FEE"
  done

  # submit a genesis validator tx
  ## Workraround for https://github.com/cosmos/cosmos-sdk/issues/8251
  (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | sourced gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID" --amount="250000000$STAKE" $KEYRING
  ## should be:
  # (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | sourced gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID"
  sourced collect-gentxs
fi
