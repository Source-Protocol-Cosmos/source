...
This section contains scripts that are useful for altering the Source project during development. They should be run from the root of the Source directory unless specified otherwise.

---

## Statesync

Statesync is a quick way to sync a Source node without having to download an entire snapshot (>100GB in most cases). Our snapshot uses the `pebbledb` database and it is automatically configured for you on run.
...
To startup 1 or 2 Source instances quickly, you will use the `test_node.sh` script like so:
