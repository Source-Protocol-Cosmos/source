...
function query_contract {
    ARGS=${3:-$SOURCED_COMMAND_ARGS}
    sourced query wasm contract-state smart $1 $2 --output json $ARGS
}
...
function wasm_cmd {
    ...
    tx_hash=$(sourced tx wasm execute $CONTRACT $MESSAGE $FUNDS $ARGS | jq -r '.txhash')
...
function mint_cw721 {
    ...
    TXMINT=$($BINARY tx wasm execute "$CONTRACT_ADDR" "$EXECUTED_MINT_JSON" $SOURCED_COMMAND_ARGS | jq -r '.txhash') && echo $TXMINT
}