#!/bin/bash
# Run this script to quickly install, setup, and run the current version of source without docker.
#
# Example:
...
export MONIKER="localsource"
export KEYALGO="secp256k1"
export KEYRING=${KEYRING:-"test"}
export HOME_DIR=$(eval echo "${HOME_DIR:-"~/.source/"}")

export BINARY=${BINARY:-sourced}
...
sourced config keyring-backend $KEYRING
...
command -v sourced > /dev/null 2>&1 || { echo >&2 "sourced command not found. Ensure this is setup / properly installed in your GOPATH."; exit 1; }
...
from_scratch () {
  ...
  # source1efd63aw40lxf3n4mhf7dzhjkr453axurv2zdzk
  ...
  # source1hj5fveer5cjtn4wd6wstzugjfdxzl0xps73ftl
  ...
  # GlobalFee
  update_test_genesis '.app_state["globalfee"]["params"]["minimum_gas_prices"]=[{"amount":"0.002500000000000000","denom":"usource"}]'

  update_test_genesis '.app_state["staking"]["params"]["bond_denom"]="usource"'  
  update_test_genesis '.app_state["bank"]["params"]["send_enabled"]=[{"denom": "usource","enabled": true}]'
  # update_test_genesis '.app_state["staking"]["params"]["min_commission_rate"]="0.100000000000000000"' # sdk 46 only   

  update_test_genesis '.app_state["mint"]["params"]["mint_denom"]="usource"'  
  update_test_genesis '.app_state["gov"]["deposit_params"]["min_deposit"]=[{"denom": "usource","amount": "1000000"}]'
  update_test_genesis '.app_state["crisis"]["constant_fee"]={"denom": "usource","amount": "1000"}'  

  update_test_genesis '.app_state["tokenfactory"]["params"]["denom_creation_fee"]=[{"denom":"usource","amount":"100"}]'

  update_test_genesis '.app_state["feeshare"]["params"]["allowed_denoms"]=["usource"]'

  # Allocate genesis accounts
  BINARY add-genesis-account $KEY 10000000usource,1000utest --keyring-backend $KEYRING
  BINARY add-genesis-account feeacc 1000000usource,1000utest --keyring-backend $KEYRING

  BINARY gentx $KEY 1000000usource --keyring-backend $KEYRING --chain-id $CHAIN_ID

  # Collect genesis tx
  BINARY collect-gentxs

  # Run this to ensure sourcerything worked and that the genesis file is setup correctly
...
BINARY start --pruning=nothing  --minimum-gas-prices=0usource --rpc.laddr="tcp://0.0.0.0:$RPC"