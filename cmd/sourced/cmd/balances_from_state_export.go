...
Example:
	sourced export-derive-balances ../genesis.json ../snapshot.json
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Produce the map of address to total source balance, both staked and UnbondingStake
			...
			for _, unbonding := range stakingGenesis.UnbondingDelegations {
				...
				unbondingSources := sdk.NewInt(0)
				for _, entry := range unbonding.Entries {
					unbondingSources = unbondingSources.Add(entry.Balance)
				}

				acc.UnbondingStake = acc.UnbondingStake.Add(unbondingSources)
			...
			for _, delegation := range stakingGenesis.Delegations {
				...
				stakedSource := delegation.Shares.MulInt(val.Tokens).Quo(val.DelegatorShares).RoundInt()

				acc.Staked = acc.Staked.Add(stakedSource)
...
Example:
	sourced staked-to-csv ../balances.json ../airdrop.csv
`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx := client.GetClientContextFromCmd(cmd)
