...
CMD bash -c printenv | grep -v "no_proxy" >>"/etc/environment" && npm install && node ./scripts/createRelayer.js --configPath $CONFIG_PATH --mnemonicPath $MNEMONIC_PATH --createClients false -z false && cron -f