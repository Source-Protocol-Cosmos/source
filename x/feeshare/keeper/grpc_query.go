...
import (
	...
	"github.com/Source-Protocol-Cosmos/source/v3/x/feeshare/types"
)
...
func (q Querier) FeeShare(
	...
	if err != nil {
		return nil, status.Errorf(
			...
			"invalid format for contract %s, should be bech32 ('source...')", req.ContractAddress,
		)
	}
...
func (q Querier) DeployerFeeShares( // nolint: dupl
	...
	if err != nil {
		return nil, status.Errorf(
			codes.InvalidArgument,
			"invalid format for deployer %s, should be bech32 ('source...')", req.DeployerAddress,
		)
	}
...
func (q Querier) WithdrawerFeeShares( // nolint: dupl
	...
	if err != nil {
		return nil, status.Errorf(
			codes.InvalidArgument,
			"invalid format for withdraw addr %s, should be bech32 ('source...')", req.WithdrawerAddress,
		)
	}
